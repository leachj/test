...
```mermaid
graph LR
    User[👤 User] -->|HTTP| Frontend[⚛️ React + TypeScript UI]
    Frontend -->|"/api requests (proxied)"| Backend[🦀 Rust + Axum API]
    Backend -->|"in-memory / shared state"| Store[(📦 App State)]
```
...
sequenceDiagram
    ...
    B->>B: Handle route & business logic
    B-->>F: JSON response
